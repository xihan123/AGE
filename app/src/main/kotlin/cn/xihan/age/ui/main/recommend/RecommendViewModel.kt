...
class RecommendViewModel @Inject constructor(
    ...
    fun getRecommendModel() = intent {
        remoteRepository.getRecommendModel().collect { recommendModel ->
            reduce {
                state.copy(
                    loading = false,
                    refreshing = false,
                    error = null,
                    currentRecommendListData = recommendModel.videos
                )
            }
        }
    }
...
data class RecommendState(
    ...
    var currentRecommendListData: List<BaseAnimeModel> = emptyList(),
) : IUiState
