...
class DescViewModel @Inject constructor(
    ...
    fun getAnimeDetailModel() = intent {
        ...
        remoteRepository.getAnimeDetailModel(animeId).collect { animeDetailModel ->
            val animeId = animeDetailModel.aniInfo.aID
            val animeCover =
                animeDetailModel.aniInfo.r封面图.ifBlank { animeDetailModel.aniInfo.r封面图小 }
            localRepository.upsertFavoriteModel(
                animeId = animeId,
                animeName = animeDetailModel.aniInfo.r动画名称,
                animeCover = animeCover,
                animeSubtitle = animeDetailModel.aniInfo.r新番标题
            )
            localRepository.upsertAnimeDetailModel(animeDetailModel.aniInfo)
            reduce {
                state.copy(
