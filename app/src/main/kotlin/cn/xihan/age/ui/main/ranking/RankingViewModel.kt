...
class RankingViewModel @Inject constructor(
    ...
    fun getRankingModel() = intent {
        remoteRepository.getRankModel(year = year).collect { rankModel ->
            reduce {
                state.copy(
                    loading = false,
                    refreshing = false,
                    error = null,
                    year = year,
                    currentRankingListData = rankModel.rank
                )
            }
        }
    }
    ...
    fun changeYear(year: String) {
        ...
        this.year = if (year == "全部") "all" else year.replace("以前","")
        getRankingModel()
    }
...
data class RankingState(
    ...
    val year: String = "all",
    var currentRankingListData: List<List<RankModel.RankItemModel>> = emptyList()
) : IUiState
