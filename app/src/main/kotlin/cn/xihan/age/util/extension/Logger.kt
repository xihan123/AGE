...
open class SimpleLoggerPrinter : LoggerPrinter {
    override fun log(level: LogLevel, tag: String, message: String, thr: Throwable?) {
        when (level) {
            LogLevel.VERBOSE -> if (thr == null) Timber.tag(tag).v(message) else Timber.tag(tag)
                .v(thr, message)

            LogLevel.DEBUG -> if (thr == null) Timber.tag(tag).d(message) else Timber.tag(tag)
                .d(thr, message)

            LogLevel.INFO -> if (thr == null) Timber.tag(tag).i(message) else Timber.tag(tag)
                .i(thr, message)

            LogLevel.WARN -> if (thr == null) Timber.tag(tag).w(message) else Timber.tag(tag)
                .w(thr, message)

            LogLevel.ERROR -> if (thr == null) Timber.tag(tag).e(message) else Timber.tag(tag)
                .e(thr, message)
        }
    }

    override fun logWtf(tag: String, message: String, thr: Throwable?) {
        if (thr == null) Timber.tag(tag).wtf(message) else Timber.tag(tag).wtf(thr, message)
    }
}
